      ...
      {grade && (
        <article className="grade-detail__card">
          <header className="grade-detail__header">
            <div className={`grade-detail__icon ${letterColorClass[grade.letter]}`} aria-hidden="true">
              {grade.letter}
            </div>
          ...
          <div className="grade-detail__terms-grid">
            <div className="grade-detail__term-card grade-detail__term-card--current">
              ...
              <p className="grade-detail__term-score">
                {grade.score}<span className="grade-detail__term-score-max"> / 100</span>
              </p>
              <div className="grade-detail__term-footer">
                ...
                {previousGrade && (
                  <ScoreDiff diff={grade.score - previousGrade.score} />
                )}
              </div>
            </div>

            <div className={`grade-detail__term-card${previousGrade ? '' : ' grade-detail__term-card--empty'}`}>
              <p className="grade-detail__term-tag">前学期</p>
              {previousGrade ? (
                <>
                  <p className="grade-detail__term-period">{prevTermPeriod(grade)}</p>
                  <p className="grade-detail__term-score">
                    {previousGrade.score}<span className="grade-detail__term-score-max"> / 100</span>
                  </p>
                  <div className="grade-detail__term-footer">
                    <span className={`grade-detail__badge ${letterColorClass[previousGrade.letter]}`}>
